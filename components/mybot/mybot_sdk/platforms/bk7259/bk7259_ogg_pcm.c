...
int bk7259_ogg_pcm_load_memory(const char *path, const uint8_t *file_data, size_t file_len,
    ...
    if (last_granule > (ogg_int64_t)pre_skip) {
        ogg_int64_t audio_samples = last_granule - (ogg_int64_t)pre_skip;
        ogg_int64_t max_samples =
            ((ogg_int64_t)MYBOT_OGG_MAX_PCM_FRAMES * 48000) / out_rate;
        if (audio_samples > max_samples) {
            MYBOT_LOGW(TAG, "ogg duration exceeds pcm limit: %s", path);
            goto cleanup;
        }
        capacity = (size_t)(audio_samples * out_rate / 48000);
    } else {
    ...
    if (capacity == 0 || capacity > MYBOT_OGG_MAX_PCM_FRAMES) {
        MYBOT_LOGW(TAG, "decoded pcm size is out of range: %s, frames=%u", path,
