...
int mybot_media_pipeline_start(mybot_media_pipeline_t *pipeline,
    ...
    /* Event-wake mpqs: cap/pb/send workers only use mpq timers + queued
     * functions (no fd watchers), so a select-mode queue would hold a 2-socket
     * UDP loopback wake pair each for nothing. SIGP_EVENT wakes on a kernel
     * event instead (0 sockets), keeping lwip UDP PCB / netconn headroom for
     * the Agora RTC channel's UDP transport. */
    pipeline->cap_mpq = aosl_mpq_create_flags(AOSL_MPQ_FLAG_SIGP_EVENT, AOSL_THRD_PRI_NORMAL,
                                              MEDIA_MPQ_STACK_SIZE, 1000, "cap_mpq",
                                              capture_worker_init, capture_worker_fini, pipeline);
    pipeline->pb_mpq = aosl_mpq_create_flags(AOSL_MPQ_FLAG_SIGP_EVENT, AOSL_THRD_PRI_NORMAL,
                                             MEDIA_MPQ_STACK_SIZE, 1000, "pb_mpq",
                                             playback_worker_init, playback_worker_fini, pipeline);
    pipeline->send_mpq = aosl_mpq_create_flags(AOSL_MPQ_FLAG_SIGP_EVENT, AOSL_THRD_PRI_NORMAL,
                                               MEDIA_MPQ_STACK_SIZE, 1000,
